...
#   2. Rebuilds in release mode via `cargo build --release` from the repo
#      root (one level up from this script). Bails on non-zero exit.
...
try {
    # Rebuild the binary in release mode.
    & cargo build --release
    ...
    Write-Output "amp-proxy restarted with $configPath; see .\run.log"
